...
local function build_visits()
  ...
  for i = 1, math.min(5, #paths) do
    ...
    name = buffers[name] > 1 and string.format('%s/%s', vim.fn.fnamemodify(paths[i], ':.:h:t'), name) or name
    ...
    table.insert(parts, highlight_text(string.format(' %d: %s ', i, name), class, state))
  end
